...
class MNISTRunner(BaseRunner):
    def __init__(self, scope, params):
        ...
        self.Writer['Random_Train_Mask'] = \
            tf.summary.FileWriter(self.Dir+'/random_train_mask', self.Sess.graph)
    ...
    def _build_lisp(self):
        ...
        for key in ['Random', 'Random_Train_Mask']:
            ...
            for copy, weight in zip(self.Tensor['Prior'][key],
                ...
                #_, sig = tf.nn.moments(tf.abs(weight), [0,1])
                #self.Output['Meta_Loss'][key] -= sig
        ...
        self.Output['Pred'] = {
            ...
            'Random_Train_Mask': self.Output['Random_Train_Mask_Pred']
        }

    def _build_outputs(self):
        with tf.variable_scope(self.scope):
            init_data = [
                {'w_init_method': 'normal', 'w_init_para': {'stddev': 0.01, 'mean': 0},
                 'b_init_method': 'constant', 'b_init_para': {'val': 0.0}}
                for _ in range(len(self.params.mlp_l_hidden_seq) + 1)
            ]
            self.Model['Snip'] = Mask('snip', self.params, 784, 10, self.params.seed)
            self.Model['Random'] = Mask('random', self.params, 784, 10,
                self.params.seed + 1562, init=init_data)
            self.Model['Random_Train_Mask'] = MaskTrain('random_train_mask',
                self.params, 784, 10, self.params.seed + 1562, init=init_data)
        ...
        self.Tensor['Random_Train_Mask_Grad'] = self.Model['Random'].Tensor['Snip_Grad']
        ...
        self.Tensor['Random_Train_Mask_Meta'] = \
            [-tf.reduce_sum(tf.abs(grad)) for grad in self.Tensor['Random_Train_Mask_Grad']]

        l1_regularizer = tf.contrib.layers.l1_regularizer(
            scale=self.params.l1_mask_penalty
        )

        #for ix, mask in enumerate(self.Model['Random_Train_Mask'].Snip['Mask_Train']):
        #    self.Tensor['Random_Train_Mask_Meta'][ix] += \
        #        tf.contrib.layers.apply_regularization(l1_regularizer, [mask])
        self.Output['Meta_Loss']['Random_Train_Mask'] = \
            tf.add_n(self.Tensor['Random_Train_Mask_Meta'])
        ...
        self.Output['Random_Train_Mask_Pred'] = self.Model['Random_Train_Mask'].run(
            self.Placeholder['Input_Feature']
        )
        ...
        self.Output['Random_Train_Mask_Loss'] = tf.reduce_mean(
            self.Tensor['Loss_Function'](
                self.Output['Random_Train_Mask_Pred'], self.Placeholder['Input_Label']
            )
        )
        ...
        self.Output['Random_Train_Mask_Train'] = \
            self.Output['Optimizer'].minimize(self.Output['Random_Train_Mask_Loss'])
    ...
    def _build_summary(self):
        ...
        self.Summary['Weight'] = {}
        for key in ['Snip', 'Random', 'Random_No_Train']: #'Random_Train_Mask']:
    ...
    def preprocess(self, features, labels):
        ...
        for ix, mask in enumerate(self.Mask['Snip']):
            self.Mask['Random'][self.Model['Random'].Snip['Mask'][ix]] = self.Mask['Snip'][mask]
        ...
        self.Mask['Random_Train_Mask'] = {}
        pretrain_mask = self.Sess.run(self.Model['Random_Train_Mask'].Snip['Mask_Train'])
        print(pretrain_mask)
        for ix, mask in enumerate(pretrain_mask):
            self.Mask['Random_Train_Mask'][self.Model['Random_Train_Mask'].Snip['Mask'][ix]] = \
                self.prune_layer(1/(1+np.exp(-mask)))
        ...
        for key in weights:
            for weight in weights[key]:
                self.Writer[key].add_summary(weight, self.params.pretrain_num_steps)
    ...
    def pretrain(self, i):
        ...
        summary, grad, comb, _ = self.Sess.run(
            [self.Summary['Meta_Loss'], self.Model['Random'].Tensor['Snip_Grad'],
             self.Model['Random'].Snip['Comb'], self.Output['Meta_Train']],
            feed_dict
        )
        ...
        for key in weights:
            for weight in weights[key]:
                self.Writer[key].add_summary(weight, i)


        print([np.sum(np.abs(c) > 1e-3) for c in comb])
        print([np.sum(np.abs(g) > 1e-5) for g in grad])
